...
const CategoryJobsPage = () => {
  ...
  if (loading) return <p className="text-center">Loading jobs for {categoryName}...</p>;
  if (error) return <p className="text-center text-red-500">Error: {error}</p>;
  if (!jobs || jobs.length === 0) return <p className="text-center">No jobs found in {categoryName}.</p>;
        ...
        {jobs.map((job) => (
          ...
          >
            ...
            <div className="space-y-1 text-sm text-gray-600 mb-3">
              ...
              <p className="flex items-center">
                <FaBriefcase className="mr-2 text-[#0077B6]" /> Salary: ₹{job.salaryMin} {job.salaryMax}/rs
              </p>
              <p className="flex items-center">
